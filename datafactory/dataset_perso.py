...
class T2SDataset(Dataset):
    def __init__(
        ...
        self.window, self.period, self.name, self.max_length = (
            window,
            period,
            name,
            max_length,
        ...
        (
            ...
            train_embedding,
            test_embedding,
        ) = self.__getsamples(self.data, self.text, self.embedding, proportion, seed)
    ...
    def __getsamples(self, data, text, embedding, proportion, seed):
        (
            ...
            train_embedding,
            test_embedding,
        ) = self.divide(data, text, embedding, proportion, seed)
        return (
            train_data,
            test_data,
            train_text,
    ...
    def read_data(name, json_root_data):
        ...
        if any(
            ...
            list_of_arrays = json_root_data["TextEmbedding"].apply(
                lambda embedding_str: [
                    float(num)
                    for num in embedding_str.replace("[", "")
                    .replace("]", "")
                    .strip()
                    .split()
                ]
...
class DatasetFeaturesSeries(Dataset):
    ...
    def __getitem__(self, idx):
        feature = torch.tensor(self.features[idx], dtype=torch.float32)  # shape [24]
