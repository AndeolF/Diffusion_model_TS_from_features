...
def initialize_ts2vec(X_train, device):
    config = dict(batch_size=8, lr=0.001, output_dims=100, max_train_length=3000)
    model = TS2Vec(input_dims=X_train.shape[-1], device=device, **config)
...
if __name__ == "__main__":
    ...
    ts_train = np.load("../Data/dataset_50_run_05s_downsample/train_time_series.npy")
    print(ts_train.shape)
    ts_train = ts_train.reshape(ts_train.shape[0], ts_train.shape[1], 1)
    fid_model = initialize_ts2vec(ts_train, "cuda")
    torch.save(
        fid_model,
        "../checkpoint/dataset_50_run_05s_downsample/fid_model.pth",
    )
